...
void PriusHybridPlugin::OnPriusCommand(const car_demo::Control::ConstPtr &msg)
{
  ...
  {
  case car_demo::Control::NEUTRAL:
    this->dataPtr->directionState = PriusHybridPluginPrivate::NEUTRAL;
    break;
  case car_demo::Control::FORWARD:
    this->dataPtr->directionState = PriusHybridPluginPrivate::FORWARD;
    break;
  case car_demo::Control::REVERSE:
...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  std::string handWheelJointName = this->dataPtr->model->GetName() + "::" + _sdf->Get<std::string>("steering_wheel");
...
void PriusHybridPlugin::Update()
{
  ...
  double brakePercent = this->dataPtr->brakePedalPercent + this->dataPtr->handbrakePercent;
  ...
  // threshold
  else if (this->dataPtr->evMode && linearVel < this->dataPtr->speedLowMedium && this->dataPtr->gasPedalPercent <= this->dataPtr->kGasPedalMediumHigh)
  {
    // Gas engine is off, running on battery
    engineOn = false;
    batteryDischargePower = throttlePower;
  ...
  if (regen)
  {
    ...
    batteryChargePower +=
        std::min(this->dataPtr->kGasPedalMediumHigh, brakePercent) * (dPtr->frontBrakeTorque * std::abs(dPtr->flWheelAngularVelocity) +
